  ...
  return (
    <Sidebar>
      ...
      <SidebarContent className="p-4 clas">
        <h2 className="text-lg font-semibold"><a className="flex items-center gap-2"
          href="/" data-discover="true"><span className="text-xl font-light text-neutral-200">OG T3 Chat</span></a></h2>
        <div className="pb-2 pt-4">
        <Link href="/chat" className="group flex flex-row items-center gap-2 text-pink-400 hover:opacity-80">
          <MessageSquarePlus className="size-4"/>
          <span className="">New Chat</span></Link>
        </div>
        <SidebarGroup className="p-0">
          <SidebarGroupLabel className="p-0 my-2">Recent Threads</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {(threadData?.threads ?? []).map((item) => (
                <SidebarMenuItem key={item.id}>
                  <SidebarMenuButton className="justify-between px-2 group/button" asChild>
      ...
      <div >
        {
          isSignedIn ? <div className="p-4 flex gap-3 h-20">
          ...
          </div> : <div className="mt-auto flex flex-row justify-between border-t border-neutral-700 p-4"><a href="/login">Login</a></div>
        }
